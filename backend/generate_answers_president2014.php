...
//answers
$fanswers = array(
  'president2014' => file("https://docs.google.com/spreadsheet/pub?key=0Ah0TJXObccZydFRGbktnVERSMGxzOUpQOGlsZVJkaVE&output=txt"),
); 
...
$a_senat = array_reverse($answers0['president2014']);
...
$answers0['president2014'] = array_reverse($a_senat);
...
foreach ($file as $row) {
  ...
  $region_code = 'president2014'; //$row_ar[0];
  $id = $row_ar[0];
  $question = trim($row_ar[4]); //mind it! (different from question generation)
...
array_shift($questions['president2014']);
...
foreach ($answers0 as $key=>$region) {
  ...
  foreach ($region[0] as $fkey => $first_row) {
    ...
    //unique code
    if (trim($first_row) == 'Bezpečnostný kód:')
...
foreach ($answers0 as $key=>$region) {
   ...
   //print_r($row);die();
    ...
    if (isset($parties[$row[$unique_code_column[$key]]])) {
      ...
      $data[$key][] = array(
        ...
        'friendly_name' => friendly_url($party['last_name'],'sk_SK.UTF-8'),
        'id' => $i,
      );
      $i++;
    } else {
  ...
  //if direcotry not existing, create it
   $dir = $path . 'volba-prezidenta-2014/';
   if(!file_exists($dir)) 
     mkdir($dir);
